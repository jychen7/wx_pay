require 'rest_client'
require 'json'
require 'cgi'
require 'securerandom'
require 'active_support/core_ext/hash/conversions'

module WxPay
  module Service
    GATEWAY_URL = 'https://api.mch.weixin.qq.com'
    OPEN_URL = 'https://open.weixin.qq.com'
    API_URL = 'https://api.weixin.qq.com'

    def self.generate_authorize_url(redirect_uri, state = nil)
      state ||= SecureRandom.hex(16)
      params = {
        redirect_uri: CGI::escape(redirect_uri),
        state: state
      }
      request_open_url("/connect/oauth2/authorize", params)
    end

    def self.authenticate(authorization_code, options = {})
      params = { code: authorization_code }
      request_auth_json("/sns/oauth2/access_token", params, options)
    end

    def self.authenticate_from_weapp(js_code, options = {})
      params = { js_code: js_code }
      request_auth_json("/sns/jscode2session", params, options)
    end
    
    INVOKE_UNIFIEDORDER_REQUIRED_FIELDS = [:body, :out_trade_no, :total_fee, :spbill_create_ip, :notify_url, :trade_type]
    def self.invoke_unifiedorder(params, options = {})
      check_required_options(params, INVOKE_UNIFIEDORDER_REQUIRED_FIELDS)

      r = request_gateway_result("/pay/unifiedorder", params, options)
      yield r if block_given?
      r
    end

    INVOKE_CLOSEORDER_REQUIRED_FIELDS = [:out_trade_no]
    def self.invoke_closeorder(params, options = {})
      check_required_options(params, INVOKE_CLOSEORDER_REQUIRED_FIELDS)

      r = request_gateway_result("/pay/closeorder", params, options)
      yield r if block_given?
      r
    end

    GENERATE_APP_PAY_REQ_REQUIRED_FIELDS = [:prepayid, :noncestr]
    def self.generate_app_pay_req(params, options = {})
      check_required_options(params, GENERATE_APP_PAY_REQ_REQUIRED_FIELDS)

      params = {
        appid: options.delete(:appid) || WxPay.appid,
        partnerid: options.delete(:mch_id) || WxPay.mch_id,
        key: options.delete(:key) || WxPay.key,
        package: 'Sign=WXPay',
        timestamp: Time.now.to_i.to_s
      }.merge(params)
      params[:sign] = WxPay::Sign.generate(params)
      params
    end

    GENERATE_JS_PAY_REQ_REQUIRED_FIELDS = [:prepayid, :noncestr]
    def self.generate_js_pay_req(params, options = {})
      check_required_options(params, GENERATE_JS_PAY_REQ_REQUIRED_FIELDS)

      params = {
        appId: options.delete(:appid) || WxPay.appid,
        package: "prepay_id=#{params.delete(:prepayid)}",
        key: options.delete(:key) || WxPay.key,
        nonceStr: params.delete(:noncestr),
        timeStamp: Time.now.to_i.to_s,
        signType: 'MD5'
      }.merge(params)
      params[:paySign] = WxPay::Sign.generate(params)
      params
    end

    INVOKE_REFUND_REQUIRED_FIELDS = [:out_refund_no, :total_fee, :refund_fee, :op_user_id]
    # out_trade_no 和 transaction_id 是二选一(必填)
    def self.invoke_refund(params, options = {})
      params[:op_user_id] ||= options.delete(:mch_id) || WxPay.mch_id

      check_required_options(params, INVOKE_REFUND_REQUIRED_FIELDS)
      warn("WxPay Warn: missing required option: out_trade_no or transaction_id must have one") if ([:out_trade_no, :transaction_id] & params.keys) == []

      r = request_gateway_result("/secapi/pay/refund", params, options.merge(ssl_cert: true))
      yield r if block_given?
      r
    end

    REFUND_QUERY_REQUIRED_FIELDS = []
    # either out_trade_no or transaction_id
    def self.refund_query(params, options = {})
      check_required_options(params, ORDER_QUERY_REQUIRED_FIELDS)
      warn("WxPay Warn: missing required option: out_trade_no or transaction_id must have one") if ([:out_trade_no, :transaction_id] & params.keys) == []

      r = request_gateway_result("/pay/refundquery", params, options)
      yield r if block_given?
      r
    end

    INVOKE_TRANSFER_REQUIRED_FIELDS = [:partner_trade_no, :openid, :check_name, :amount, :desc, :spbill_create_ip]
    def self.invoke_transfer(params, options = {})
      check_required_options(params, INVOKE_TRANSFER_REQUIRED_FIELDS)

      r = request_gateway_result(
        "/mmpaymkttransfers/promotion/transfers",
        params,
        options.merge(ssl_cert: true, appid_variable: :mch_appid, mchid_variable: :mchid)
      )
      yield r if block_given?
      r
    end

    GETTRANSFERINFO_FIELDS = [:partner_trade_no]
    def self.gettransferinfo(params, options = {})
      check_required_options(params, GETTRANSFERINFO_FIELDS)

      r = request_gateway_result("/mmpaymkttransfers/gettransferinfo", params, options.merge(ssl_cert: true))
      yield r if block_given?
      r
    end

    INVOKE_REVERSE_REQUIRED_FIELDS = []
    # either out_trade_no or transaction_id
    def self.invoke_reverse(params, options = {})
      check_required_options(params, INVOKE_REVERSE_REQUIRED_FIELDS)
      warn("WxPay Warn: missing required option: out_trade_no or transaction_id must have one") if ([:out_trade_no, :transaction_id] & params.keys) == []

      r = request_gateway_result("/secapi/pay/reverse", params, options.merge(ssl_cert: true))
      yield r if block_given?
      r
    end

    INVOKE_MICROPAY_REQUIRED_FIELDS = [:body, :out_trade_no, :total_fee, :spbill_create_ip, :auth_code]
    def self.invoke_micropay(params, options = {})
      check_required_options(params, INVOKE_MICROPAY_REQUIRED_FIELDS)

      r = request_gateway_result("/pay/micropay", params, options.merge(ssl_cert: true))
      yield r if block_given?
      r
    end

    ORDER_QUERY_REQUIRED_FIELDS = []
    # either out_trade_no or transaction_id
    def self.order_query(params, options = {})
      check_required_options(params, ORDER_QUERY_REQUIRED_FIELDS)
      warn("WxPay Warn: missing required option: out_trade_no or transaction_id must have one") if ([:out_trade_no, :transaction_id] & params.keys) == []
      
      r = request_gateway_result("/pay/orderquery", params, options)
      yield r if block_given?
      r
    end

    DOWNLOAD_BILL_REQUIRED_FIELDS = [:bill_date]
    def self.download_bill(params, options = {})
      check_required_options(params, DOWNLOAD_BILL_REQUIRED_FIELDS)
      
      r = request_gateway_raw("/pay/downloadbill", params, options)
      yield r if block_given?
      r
    end

    SETTLEMENT_QUERY_REQUIRED_FIELDS = [:usetag, :offset, :limit, :date_start, :date_end]
    def self.settlement_query(params, options = {})
      check_required_options(params, SETTLEMENT_QUERY_REQUIRED_FIELDS)
      
      r = request_gateway_result("/pay/settlementquery", params, options)
      yield r if block_given?
      r
    end

    def self.sendgroupredpack(params, options={})
      r = request_gateway_result(
        "/mmpaymkttransfers/sendgroupredpack",
        params,
        options.merge(ssl_cert: true, appid_variable: :wxappid)
      )
      yield r if block_given?
      r
    end

    def self.sendredpack(params, options={})
      r = request_gateway_result(
        "/mmpaymkttransfers/sendredpack",
        params,
        options.merge(ssl_cert: true, appid_variable: :wxappid)
      )
      yield r if block_given?
      r
    end

    class << self
      private

      def check_required_options(options, names)
        return unless WxPay.debug_mode?
        names.each do |name|
          warn("WxPay Warn: missing required option: #{name}") unless options.has_key?(name)
        end
      end

      def generate_signed_payload(params)
        sign = WxPay::Sign.generate(params)
        params.delete(:key) if params[:key]
        body = params.map{|k, v| "<#{k}>#{v}</#{k}>"}.join
        "<xml>#{body}<sign>#{sign}</sign></xml>"
      end

      def request_gateway_raw(path, params, options = {})
        appid_variable = options[:appid_variable] || :appid
        mchid_variable = options[:mchid_variable] || :mch_id

        default_params = {
          mch_id: options.delete(:mch_id) || WxPay.mch_id,
          key: options.delete(:key) || WxPay.key,
          nonce_str: SecureRandom.uuid.tr('-', ''),
        }
        default_params[appid_variable] = options.delete(:appid) || WxPay.appid
        default_params[mchid_variable] = options.delete(:mch_id) || WxPay.mch_id
        params = params.reverse_merge(default_params)
        
        if options[:ssl_cert]
          options = {
            ssl_client_cert: options.delete(:apiclient_cert) || WxPay.apiclient_cert,
            ssl_client_key: options.delete(:apiclient_key) || WxPay.apiclient_key,
            verify_ssl: OpenSSL::SSL::VERIFY_NONE
          }.merge(options)
        end
        options.delete(:ssl_cert)
        
        options = WxPay.extra_rest_client_options.merge(options)
        RestClient::Request.execute({
          method: :post,
          url: "#{GATEWAY_URL}" + path,
          payload: generate_signed_payload(params),
          headers: { content_type: 'application/xml' }
        }.merge(options))
      end

      def request_gateway_result(path, params, options = {})
        raw = request_gateway_raw(path, params, options)
        WxPay::Result.new(Hash.from_xml(raw))
      end

      def request_auth_json(path, params, options = {})
        params = {
          appid: WxPay.appid,
          secret: WxPay.appsecret,
          grant_type: "authorization_code"
        }.merge(params)
        url = request_url(API_URL + path, params)
        options = WxPay.extra_rest_client_options.merge(options)

        ::JSON.parse(RestClient::Request.execute(
          {
            method: :get,
            url: url
          }.merge(options)
        ), quirks_mode: true)
      end

      def request_open_url(path, params)
        params = {
          appid: WxPay.appid,
          response_type: "code",
          scope: "snsapi_base"
        }.merge(params)
        request_url(OPEN_URL + path, params).to_s
      end

      def request_url(url, params)
        url + "?" + params.to_query
      end
    end
  end
end
